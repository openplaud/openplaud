...
- `factory.ts` — `createStorageProvider()` (env-based) and `createUserStorageProvider(userId)` (reads per-user DB config — **use this in API routes**)
...
- Original Plaud files: numeric/UUID IDs from the Plaud cloud API
- Locally uploaded files: `uploaded-{nanoid}`
- Split segments: `split-{originalId}-part001`, `split-{originalId}-part002`, …
- Silence-removed copies: `silence-removed-{originalId}`

Use `isPlaudLocallyCreated()` (`src/lib/plaud/sync-title.ts`) to test for any locally-created prefix. The UI and sync-title API use these prefixes to determine whether a recording can be synced back to the Plaud device.
