...
export function removeRepetitions(text: string): string {
    ...
    for (const [minW, maxW, minReps] of tiers) {
        for (let w = minW; w <= maxW; w++) {
            ...
            // Scan forward from the start; only remove trailing repetitions
            // (the condition at the end ensures the repeated phrase runs to
            // the end of the text), so legitimate repeated content earlier
            // in the text is left intact.
